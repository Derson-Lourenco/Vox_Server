            ...
            connection.query(
                ...
                async (error, results) => {
                    ...
                    const licitacoesPromises = idsPredefinidos.map(async idUnidadeGestora => {
                        ...
                        const totalLicitacoes = licitacoesResponse.data.length;
                        console.log(`Total de licitações para ID ${idUnidadeGestora}: ${totalLicitacoes}`);
    
                        return { idUnidadeGestora, licitacoes: licitacoesResponse.data, totalLicitacoes };
                    });
                    ...
                    // Somando o total de licitações de todos os municípios
                    const totalLicitacoesGeral = licitacoesResults.reduce((total, { totalLicitacoes }) => total + totalLicitacoes, 0);
                    console.log('Total de licitações de todos os municípios:', totalLicitacoesGeral);
                    ...
                    // Retornar os dados combinados e o total de licitações
                    res.json({
                        detalhes: detalhesFlattened,
                        totalLicitacoesGeral: totalLicitacoesGeral // Incluindo o total de licitações
                    });
                }
            );
        } catch (error) {
